...
import {
  generateApiUrl,
  generateHttpOptions,
} from '../utils/api-functions/api-functions';
...
export class ContactsService {
  filteredContactsSubject = new BehaviorSubject<any>([]);
  ...
  getContacts(): Observable<{ message: string; contacts: IContact[] }> {
    ...
    return this.http.get<{ message: string; contacts: IContact[] }>(
      getContactsApiUrl,
      httpOptions
    );
  }
  ...
  removeContact(
    contact: IContact
  ): Observable<{ message: string; deleted: IContact }> {
    const removeContactApiUrl: string = generateApiUrl(
      'contacts',
      'remove',
      contact._id
    );
    return this.http.delete<{ message: string; deleted: IContact }>(
      removeContactApiUrl,
      httpOptions
    );
  }
  ...
  handleRemove(
    response: { message: string; deleted: IContact },
    DBContacts: IContact[],
    filteredContacts: IContact[]
  ): [IContact[], IContact[]] {
    this.toastr.success(response.message, 'Contact Deleted');
    filteredContacts = filteredContacts.filter(
      (contact: IContact) => contact._id !== response.deleted._id
    );
    DBContacts = DBContacts.filter(
      (contact: IContact) => contact._id !== response.deleted._id
    );
  ...
  addContact(
    contact: IContact
  ): Observable<{ message: string; new: IContact }> {
    const addContactApiUrl: string = generateApiUrl('contacts', 'add');
    return this.http.post<{ message: string; new: IContact }>(
      addContactApiUrl,
      contact,
      httpOptions
    );
  ...
  updateContact(
    updatedDetails: IContact,
    contactId: string
  ): Observable<{ message: string; updated: IContact }> {
    const updateContactApiUrl: string = generateApiUrl(
      'contacts',
      'update',
      contactId
    );
    return this.http.put<{ message: string; updated: IContact }>(
      updateContactApiUrl,
      updatedDetails,
      httpOptions
    );
  ...
  toggleLikeContact(
    contact: IContact
  ): Observable<{ message: string; liked?: IContact; unliked?: IContact }> {
    const likeContactApiUrl: string = generateApiUrl(
      'contacts',
      contact.liker ? 'unlike' : 'like',
      contact._id
    );
    return this.http.put<{
      ...
      liked?: IContact;
      unliked?: IContact;
    }>(likeContactApiUrl, '', httpOptions);
  }

  handleToggleLike(
    response: { message: string; liked?: IContact; unliked?: IContact },
    DBContacts: IContact[],
    filteredContacts: IContact[]
  ): [IContact[], IContact[]] {
    DBContacts = DBContacts.map((contact: IContact) => {
      if (response.liked)
        return contact._id === response.liked?._id ? response.liked : contact;
      else
        return contact._id === response.unliked?._id
          ? response.unliked
          : contact;
    });
