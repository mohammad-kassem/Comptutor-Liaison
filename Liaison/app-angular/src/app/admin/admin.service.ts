...
export class AdminService {
  ...
  filterUsers(
    serachText: string
  ): Observable<{ count: number; users: IUser[] }> {
    const filterUsersApiUrl: string = generateApiUrl(
      'users',
      '',
      serachText,
      'search'
  ...
  getContacts(
    userId: string
  ): Observable<{ count: number; contacts: IContact[] }> {
    const getContactsApiUrl: string = generateApiUrl(
      'contacts',
      'admin',
      userId
    );
    return this.http.get<{ count: number; contacts: IContact[] }>(
      getContactsApiUrl,
      httpOptions
    );
  }
  ...
  getMessages(
    userId: string
  ): Observable<{ count: number; messages: IMessage[] }> {
    const getMessagesApiUrl: string = generateApiUrl(
      'messages',
      'admin',
      userId
    );
    return this.http.get<{ count: number; messages: IMessage[] }>(
      getMessagesApiUrl,
      httpOptions
    );
  }
  ...
  refactorByDate(data: IContact[] | IMessage[]) {
    ...
    for (let dataItem of data) {
      ...
      for (let seriesItem of series)
        month === seriesItem.name && year === currentYear && seriesItem.value++;
    }
    return series;
  }
